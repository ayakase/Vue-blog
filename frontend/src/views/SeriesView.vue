<template>
  <div class="container">
    <div class="tag-container" v-if="seriesArray">
      <v-card
        v-for="series in seriesArray"
        :key="series"
        class="individual-tag"
        @click="goToPost(series.id)"
      >
        <v-card-title class="post-title" @click.prevent="">
          {{ series.title }}
        </v-card-title>
        <v-card-subtitle> {{ series.author.name }} Posts </v-card-subtitle>
  ...
  setup() {
    ...
    function goToTagPage(page) {
      ...
      api
        .get("/allseries/" + page)
        .then((response) => {
          seriesArray.value = response.data.rows;
